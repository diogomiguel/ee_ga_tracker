...
class Ga_tracker
{
    ...
    function __construct()
    {
        ...
        if (!isset(self::$_ga_key)) {
            ...
            $query = $this->EE->db->select('google_analytics_key')->where('site_id', $this->EE->config->item('site_id'))->get('gatracker_config');
    ...
    function pageview_link()
    {
        ...
        return $this->EE->functions->fetch_site_index() . QUERY_MARKER . 'ACT=' . $this->EE->functions->fetch_action_id('Ga_tracker', 'track_pageview') . AMP . 'page=' . $page . AMP . 'title=' . $title . AMP . 'rurl=' . $rurl;
    }
    ...
    function event_link()
    {
        ...
        return $this->EE->functions->fetch_site_index() . QUERY_MARKER . 'ACT=' . $this->EE->functions->fetch_action_id('Ga_tracker', 'track_event') . AMP . 'event=' . $event . AMP . 'category=' . $category . AMP . 'label=' . $label . AMP . 'rurl=' . $rurl;
    }
